import {
  ...
  import { toggleMaxStats } from "./SHD.js"
  ...
  function updateInventoryTiles() {
    // First, remove selected class from all tiles
    document.querySelectorAll(".inventory-tile").forEach((tile) => {
      tile.classList.remove("selected")
    })
  
    // Then update tiles with saved loadout items
    for (const [slot, item] of Object.entries(currentLoadout)) {
      ...
      if (tile) {
        ...
        tile.classList.add("selected") // Mark as selected
      }
    }
  }
  ...
  async function showItemSelection(slot, type) {
    ...
    // Get items based on type and slot
    let items = []
  
    try {
      items = await getItems(type, slot)
      console.log(`Loaded ${items.length} items for ${slot} (${type})`)
    } catch (error) {
      console.error(`Error loading items for ${slot} (${type}):`, error)
      items = []
    }
  
    const currentItemTypeElement = document.getElementById("currentItemType")
    currentItemTypeElement.textContent = `Select ${slot.charAt(0).toUpperCase() + slot.slice(1)}`
    ...
    if (currentItemType === "weapons") {
      ...
      if (item.brand) {
        ...
        if (gearSet) {
          ...
          brandInfo = `<div class="item-brand gear-set">${gearSet.name}</div>`
        }
      }
  ...
  function saveItemToLoadout(item) {
    ...
    // Mark as selected
    document.querySelectorAll(".inventory-tile").forEach((t) => {
      if (t === tile) {
